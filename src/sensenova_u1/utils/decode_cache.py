...
class PreallocatedDecodeLayer(CacheLayerMixin):
    ...
    def advance_length(self, added: int) -> None:
        ...
        if end > self.max_cache_len:
            raise RuntimeError(f"decode cache exhausted at {end}/{self.max_cache_len} tokens")
        self._length = end
        self._refresh_views()
    ...
    def update(
        ...
        if end > self.max_cache_len:
            raise RuntimeError(f"decode cache exhausted at {end}/{self.max_cache_len} tokens")
        self.flash_decode_k_cache[:, self._length : end].copy_(key_states.transpose(1, 2))
        self.flash_decode_v_cache[:, self._length : end].copy_(value_states.transpose(1, 2))
        self._length = end
        self._refresh_views()
...
class PreallocatedDecodeCache(Cache):
    ...
    def from_cache(cls, cache: Cache, *, capacity: int) -> PreallocatedDecodeCache:
        ...
        seqlens = torch.tensor([prefix], dtype=torch.int32, device=first.keys.device)
        layers = [
...
def reserve_cuda_graph_decode(
    ...
    *,
    ...
    if capacity > context_limit:
        raise ValueError(f"decode cache needs {capacity} tokens, context is {context_limit}")
    workspace = getattr(owner, "_cuda_graph_decode_workspace", None)
    if workspace is None or not workspace.cache.can_load(cache, required_capacity=capacity):
    ...
    prepare_rotary = getattr(model.language_model.model, "prepare_rotary_inference_cache", None)
    ...
    prepare_rotary(
        ...
        max_spatial_position=int(model.language_model.config.max_position_embeddings_hw),
        device=device,
        dtype=dtype,
    )
    return workspace
