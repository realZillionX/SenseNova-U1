...
def _make_entry(name, path, owner, target_spec):
    ...
    if target_shape is not None and list(tensor.shape) != list(target_shape):
        if tensor.ndim == 2 and list(tensor.t().shape) == list(target_shape):
            ...
            raise RuntimeError(
                f"checkpoint/runtime shape mismatch for {name}: {list(tensor.shape)} != {list(target_shape)}"
            )
...
def _scan_manifest(
    ...
    missing = {consumer: sorted(closure - seen_closures[consumer]) for consumer, closure in closures.items()}
...
def _select_resume_tensors(model_path: Path, closure_specs):
    ...
    language_norms = sorted(name for name in language_names if ".self_attn.k_norm.weight" in name)
    ...
    for consumer in ("vision", "x2v"):
        candidates = [
            ...
            if spec.get("shape") is not None and _torch_dtype(str(spec["dtype"])).is_floating_point
        ]
        if not candidates:
...
def _build_buckets(entries, max_bytes=256 * 1024 * 1024):
    ...
    for index, entry in enumerate(entries):
        ...
        if current and (
            entry["dtype"] != current_dtype or entry["owner"] != current_owner or current_bytes + size > max_bytes
        ):
    ...
    for bucket_index, indices in enumerate(buckets):
        flat = torch.cat([_load_entry_tensor(entries[index]).reshape(-1) for index in indices]).contiguous()
        ...
        for consumer, group in groups.items():
            for bucket in buckets:
                if consumer not in bucket.get("consumers", [consumer]):
                    continue
                tensor = torch.cat(
                    [_load_entry_tensor(entries[index]).reshape(-1) for index in bucket["entry_indices"]]
                ).contiguous()
...
def _closure_from_init(receipt):
    ...
    return {
        consumer: {name: data.get("closure_specs", {}).get(name, {}) for name in data["closure_names"]}
...
def main():
    ...
    parser.add_argument("--output-dir", default="/tmp/sensenova-u15-forge-rl-smoke")
