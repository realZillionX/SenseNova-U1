...
def packed_collate_fn(batch, packed_length):
    ...
    xs, ys, cu_seqlens, indexes, ts, ss = [], [], [], [], [], []
    ...
    for b in batch:
        ...
        if b.get("subset_ids", None) is not None:
            compute_subset_loss = True
            ss.append(torch.LongTensor(b["subset_ids"]))
    ...
    if compute_subset_loss:
        ss = torch.nn.utils.rnn.pad_sequence(ss, batch_first=True, padding_value=0)
    ...
    inputs = {"input_ids": xs, "cu_seqlens": cu_seqlens, "indexes": indexes, "type_ids": ts}
    if compute_subset_loss:
        inputs["subset_ids"] = ss
