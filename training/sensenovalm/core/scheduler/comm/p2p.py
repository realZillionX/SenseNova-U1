...
# ColossalAI (HPC-AI Tech, Apache-2.0) with portions from Megatron-LM (NVIDIA).
# -*- encoding: utf-8 -*-
...
def get_none_tensor():
    ...
    if NONE_TENSOR is None:
        NONE_TENSOR = torch.tensor(-1234, dtype=torch.int64, device=get_current_device())
...
def create_recv_buffer_with_shapes(recv_shapes, dtypes, requires_grads, scatter_gather_tensors):
    if isinstance(recv_shapes, torch.Size):
        ...
        buffer_recv = torch.empty(
            recv_chunk_shape,
            requires_grad=requires_grads if requires_grads is not None else False,
            device=get_current_device(),
            dtype=dtypes,
        )
        return buffer_recv, recv_split
    buffer_recv, recv_split_list = [], []
    for idx, recv_shape in enumerate(recv_shapes):
        recv_chunk_shape, recv_split = _get_tensor_shape(recv_shape, scatter_gather_tensors)
        tensor_recv = torch.empty(
            recv_chunk_shape,
            requires_grad=requires_grads[idx] if requires_grads is not None else False,
            device=get_current_device(),
            dtype=dtypes[idx],
        )
        buffer_recv.append(tensor_recv)
        recv_split_list.append(recv_split)
    return buffer_recv, recv_split_list


def process_object_to_send(object_send, scatter_gather_tensors):
    if object_send is None:
        return get_none_tensor()

    if isinstance(object_send, torch.Tensor):
        object_send = object_send.contiguous()
    ...
    for tensor_send in object_send:
        if tensor_send is None:
            tensor_send = get_none_tensor()
            object_send_list.append(tensor_send)
        else:
            tensor_send = tensor_send.contiguous()
...
def filling_ops_queue(obj, comm_op, comm_rank, ops_queue):
    if isinstance(obj, torch.Tensor):
        op_to_add = dist.P2POp(comm_op, obj.contiguous(), comm_rank)
        ops_queue.append(op_to_add)
    else:
        for tensor_to_comm in obj:
            op_to_add = dist.P2POp(comm_op, tensor_to_comm.contiguous(), comm_rank)
            ops_queue.append(op_to_add)
...
def _communicate(
    ...
    prev_rank: int = None,
    next_rank: int = None,
    dtypes_prev: Union[torch.dtype, List[torch.dtype]] = None,
    dtypes_next: Union[torch.dtype, List[torch.dtype]] = None,
    requires_grads_prev: List[bool] = None,
    requires_grads_next: List[bool] = None,
    ...
    if recv_prev:
        ...
        tensor_recv_prev, recv_prev_split = create_recv_buffer_with_shapes(
            recv_prev_shape, dtypes_prev, requires_grads_prev, scatter_gather_tensors
        )

    if recv_next:
        assert recv_next_shape is not None
        tensor_recv_next, recv_next_split = create_recv_buffer_with_shapes(
            recv_next_shape, dtypes_next, requires_grads_next, scatter_gather_tensors
        )
        ...
        if isinstance(tensor_recv_next, torch.Tensor):
            if recv_next_split:
                tensor_recv_next = (
                    ...
                    .requires_grad_(tensor_recv_next.requires_grad)
                )
        else:
            for index in range(len(tensor_recv_next)):
                if recv_next_split[index]:
                    tensor_recv_next[index] = (
                        gather_split_1d_tensor(tensor_recv_next[index])
                        .view(recv_next_shape[index])
                        .requires_grad_(tensor_recv_next[index].requires_grad)
                    )

    # process tensor is None
    if isinstance(tensor_recv_prev, torch.Tensor) and torch.equal(tensor_recv_prev, get_none_tensor()):
        tensor_recv_prev = None
    if tensor_recv_prev is not None and not isinstance(tensor_recv_prev, torch.Tensor):
        for index in range(len(tensor_recv_prev)):
            if torch.equal(tensor_recv_prev[index], get_none_tensor()):
                tensor_recv_prev[index] = None

    if isinstance(tensor_recv_next, torch.Tensor) and torch.equal(tensor_recv_next, get_none_tensor()):
        tensor_recv_next = None
    if tensor_recv_next is not None and not isinstance(tensor_recv_next, torch.Tensor):
        for index in range(len(tensor_recv_next)):
            if torch.equal(tensor_recv_next[index], get_none_tensor()):
                tensor_recv_next[index] = None

    return tensor_recv_prev, tensor_recv_next


def recv_forward(
    input_tensor_shape, prev_rank=None, dtypes=None, requires_grads=None, scatter_gather_tensors=False
    ...
    input_tensor, _ = _communicate(
        ...
        prev_rank=prev_rank,
        dtypes_prev=dtypes,
        requires_grads_prev=requires_grads,
...
def recv_backward(
    output_grad_shape, next_rank=None, dtypes=torch.float, scatter_gather_tensors=False
) -> Union[torch.Tensor, List[torch.Tensor]]:
...
def send_backward(input_tensor_grad, prev_rank=None, scatter_gather_tensors=False) -> None:
    ...
    if input_tensor_grad is None:
        input_tensor_grad = get_none_tensor()
    _communicate(object_send_prev=input_tensor_grad, prev_rank=prev_rank, scatter_gather_tensors=scatter_gather_tensors)


def send_forward_recv_backward(
    output_tensor, output_grad_shape, next_rank=None, dtypes=torch.float, scatter_gather_tensors=False
) -> Union[torch.Tensor, List[torch.Tensor]]:
...
def send_forward_backward_recv_forward_backward(
    ...
    """
    input_tensor, output_tensor_grad = _communicate(  # pylint: disable=E1123
        object_send_next=output_tensor,
        object_send_prev=input_tensor_grad,
        recv_prev=input_tensor_shape is not None,
        recv_next=output_grad_shape is not None,
...
def send_backward_and_recv_next_backward_async(
    ...
    if recv_next_shape is not None:
        ...
        # create receive buffer
        tensor_recv_next, recv_next_split = create_recv_buffer_with_shapes(
            recv_next_shape, dtype, True, scatter_gather_tensors
        )
