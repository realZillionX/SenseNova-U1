...
from sensenovalm.core.gradient_handler import BaseGradientHandler
from sensenovalm.core.model_average import AveragedModel
from sensenovalm.solver.optimizer.hybrid_zero_optim import BaseOptimizer
from sensenovalm.solver.schedulers.beta2_scheduler import Beta2Scheduler
from sensenovalm.utils.common import get_batch_size, move_to_device
...
class Engine:
    ...
    Args:
        ...
        lr_scheduler (torch.optim.lr_scheduler._LRScheduler, optional): The learning rate scheduler for the optimizer.
                                                                        Default is None.
        beta2_scheduler (sensenovalm.solver.beta2_scheduler.Beta2Scheduler, optional): The beta2 scheduler for the
    ...
    Examples:
        ...
        >>> optimizer = ...
        >>> train_dataloader = ...
        >>> engine, _, _, _ = sensenovalm.initialize_engine(model, optimizer, criterion)
        >>> engine.train()
    ...
    def __init__(
        ...
        *,
        ...
        # Optional averaged model (safe: our AveragedModel does NOT deepcopy(model))
        self._averaged_model = averaged_model
    ...
    def averaged_model(self):
        """Returns averaged model (if enabled), otherwise None."""
    ...
    def step(self):
        ...
        if success and self._averaged_model is not None:
            self._averaged_model.update_parameters(self.model.model)
    ...
    def backward_by_grad(self, tensor, grad):
        ...
        if isinstance(tensor, torch.Tensor) and tensor.requires_grad is False:
            return
        if isinstance(tensor, (tuple, List)) and tensor[0].requires_grad is False:
            return
