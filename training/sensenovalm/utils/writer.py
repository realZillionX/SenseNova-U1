    ...
    if gpc.get_local_rank(ParallelMode.DATA) == 0:
        writer.add_text(
            ...
            text_string=f"file_path={tb_logdir} hostname={socket.gethostname()} \
                        device={sensenovalm_accelerator.get_device_id()}",
            global_step=step_count,
        )
...
class Writer:
    ...
    Args:
        ...
        enable_wandb (bool): A boolean indicating whether to enable the wandb writer.
    ...
    def __init__(
        ...
        self.logger = logger
        self.enable_wandb = enable_wandb and gpc.is_rank_for_log()
        ...
        if self.enable_wandb:
            try:
                ...
                init_kwargs = {
                    "project": os.environ.get("WANDB_PROJECT", "NEO_Unify"),
                    "name": job_name,
                }
                init_timeout = os.environ.get("WANDB_INIT_TIMEOUT")
                if init_timeout is not None:
                    try:
                        init_kwargs["settings"] = wandb.Settings(init_timeout=float(init_timeout))
                    except ValueError:
                        self._log_warning(
                            f"Invalid WANDB_INIT_TIMEOUT={init_timeout!r}, fallback to wandb default timeout."
                        )
                ...
                self._disable_wandb(
                    "wandb.init failed, disable wandb and continue with tensorboard only.\n"
    ...
    def _disable_wandb(self, reason: str):
            ...
            if not isinstance(value, str):
                if 'loss' in key.lower():
                    try:
                        ...
                        self._disable_wandb(
                            "wandb.log failed, disable wandb and continue with tensorboard only.\n"
                            f"{traceback.format_exc()}"
                        )
