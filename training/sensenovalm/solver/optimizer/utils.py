...
# Derived from InternEvo (OpenGVLab, Apache-2.0). Gradient-norm and tensor
# (un)flatten patterns follow Megatron-LM / DeepSpeed conventions.
# -*- encoding: utf-8 -*-
...
from sensenovalm.utils.common import get_current_device, get_tensor_norm, move_norm_to_cuda
...
def compute_norm(gradients, parameters, norm_type=2, zero_mode=ParallelMode.ZERO1):
    ...
    if norm_type == inf:
        ...
        3. For the IS_WEIGHT_ZERO_PARALLEL parameter group, gradients along the wp+pp+zero dimensions
            from all ranks should be aggregated.
        4. For the IS_WEIGHT_EXPERT_DATA_PARALLEL parameter group, gradients along the ewp+pp+zero+ep dimensions
            from all ranks should be aggregated.
        """
        if is_tensor_zero_parallel_parameter(parameters[0]):
            if gpc.is_using_parallel_mode(ParallelMode.TENSOR):
                dist.all_reduce(total_norm, op=dist.ReduceOp.SUM, group=gpc.get_group(ParallelMode.TENSOR))
        elif is_weight_expert_data_parallel_parameter(parameters[0]):
            if gpc.is_using_parallel_mode(ParallelMode.EXPERT_WEIGHT):
                dist.all_reduce(total_norm, op=dist.ReduceOp.SUM, group=gpc.get_group(ParallelMode.EXPERT_WEIGHT))
        else:
            if gpc.is_using_parallel_mode(weight_parallel_mode):
                dist.all_reduce(
                    total_norm,
...
class DynamicGradScaler(BaseGradScaler):
    ...
    def _sanity_checks(self) -> None:
        ...
        if self._dtype == torch.float16:
            ...
            # assert self._backoff_factor < 1.0 and self._backoff_factor > 0,
