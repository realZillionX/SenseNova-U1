...
# Derived from InternEvo (OpenGVLab, Apache-2.0). Activation-checkpointing
# logic follows the `torch.utils.checkpoint` pattern with custom RNG handling.
# -*- encoding: utf-8 -*-
...
from sensenovalm.core.context.random import (
    get_current_mode,
    get_states,
    set_mode,
    set_seed_states,
...
class CheckpointFunction(torch.autograd.Function):
    ...
    def backward(ctx, *args):
        ...
        if ctx.had_autocast_in_fwd:
            with torch.enable_grad(), sensenovalm_accelerator.amp.autocast():
    ...
    def unpack(x):
        ...
        if len(storage) == 0:
            ...
            if has_autocast_in_fwd:
                with torch.enable_grad(), sensenovalm_accelerator.amp.autocast(), torch.autograd.graph.saved_tensors_hooks(
