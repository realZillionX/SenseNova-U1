...
class SftValidationTest(unittest.TestCase):
    ...
    def test_each_sample_and_noise_seed_has_equal_weight(self):
        rows = [dict(sample_id=identity, noise_seed=seed, text_loss=text, image_loss=image)
                for identity, seed, text, image in
                [('b', 2, 10., 4.), ('a', 1, 2., 1.), ('b', 1, 6., 2.), ('a', 2, 4., 3.)]]
        report = aggregate_samples(rows, ['a', 'b'], [1, 2])
        self.assertEqual(report['means'], dict(text_loss=5.5, image_loss=2.5, loss=8.))
        self.assertEqual(report['examples'][0]['loss'], 5.)
