...
class Reader(LazySupervisedDataset):
    ...
    def get_sample(self, index):
        ...
        return dict(input_ids=torch.full((size,), 7), labels=torch.full((size,), 7),
                    type_ids=torch.zeros(size, dtype=torch.long), pixel_values=None,
...
class FiniteEpochTest(unittest.TestCase):
    def test_two_modalities_flush_uneven_rank_tails_without_restarting(self):
        ...
        for factor in (1, 3):
            ...
            for rank in range(3):
                ...
                with patch.object(packing, 'gpc', SimpleNamespace(config=SimpleNamespace(data={}))), \
                     patch.object(packing, 'get_rank', return_value=rank), \
                     patch.object(packing, 'get_world_size', return_value=3):
                    dataset = PackedDataset(tokenizer, rank, 3, [reader],
                                            max_packed_tokens=20, max_buffer_size=10,
                                            replacement=False, allow_overflow=False)
                    rows = list(dataset)
    ...
    def test_exhaustion_does_not_restart_when_replacement_is_disabled(self):
        ...
        packer.dataset_weight = packer.dataset_weight_orig = [1.]
        self.assertIsNone(packer.next_data(0))
        self.assertEqual(reader.starts, 1)
        self.assertEqual(packer.dataset_weight, [0.])
