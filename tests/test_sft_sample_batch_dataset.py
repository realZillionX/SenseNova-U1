...
class Reader:
    ...
    def get_sample(self, row):
        ...
        return {
            "sample_ids": [str(value["sample_id"])],
            "input_ids": torch.full((length,), 7),
            "pixel_values": None,
...
def collate(rows, *, max_item_length):
    ...
    data = {
        "input_ids": torch.nn.functional.pad(row["input_ids"], (0, padding)).unsqueeze(0),
        "sample_ids": row["sample_ids"],
        "num_samples": count,
        "samples_per_microbatch": [count],
