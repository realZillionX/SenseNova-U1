...
class DecodeCacheTest(unittest.TestCase):
    def _prefix(self, length: int = 3, device: str | torch.device = "cpu") -> DynamicCache:
        cache = DynamicCache()
        for layer in range(2):
            keys = torch.arange(2 * length * 4, dtype=torch.float32, device=device).reshape(1, 2, length, 4)
    ...
    def test_graph_capture_uses_each_workspace_device(self) -> None:
        ...
        with warnings.catch_warnings(record=True) as observed:
            ...
            for index in range(2):
                ...
                cache = PreallocatedDecodeCache.from_cache(self._prefix(device=device), capacity=8)
                workspace = CudaGraphDecodeWorkspace(model=model, cache=cache)
                output = workspace.replay(torch.tensor([2], device=device), t_index=3)
                torch.cuda.synchronize(device)
                self.assertEqual(output.device, device)
                self.assertEqual(cache.get_seq_length(), 4)
        self.assertFalse(any("CUDA Graph is empty" in str(warning.message) for warning in observed))
