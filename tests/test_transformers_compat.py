...
class TransformersCompatibilityTest(unittest.TestCase):
    ...
    def test_dense_model_shares_rope_and_skips_zero_spatial_decode(self) -> None:
        ...
        calls = {(layer_index, axis): 0 for layer_index in range(config.num_hidden_layers) for axis in ("t", "hw")}
        handles = []
    ...
    def test_cached_multitoken_flash_attention_matches_explicit_causal_mask(self) -> None:
        ...
        prefix_mask = torch.triu(torch.full((1, 1, 3, 3), -torch.inf, device="cuda"), diagonal=1)
        ...
        dense_mask[:, :, :, 3:] = torch.triu(torch.full((2, 2), -torch.inf, device="cuda"), diagonal=1)
        try:
            with torch.no_grad():
