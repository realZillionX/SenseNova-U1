...
class TrainingRMSNormTest(unittest.TestCase):
    ...
    def test_flash_cuda_path_matches_manual_forward_and_backward(self) -> None:
        ...
        reference = norm_ops.manual_rms_norm(inputs, weight, torch.Size((128,)), 1e-6)
        ...
        torch.testing.assert_close(fused_inputs.grad, inputs.grad, rtol=3e-2, atol=3e-2)
        ...
        relative_weight_error = (
            fused_weight.grad.float() - weight.grad.float()
        ).norm() / weight.grad.float().norm().clamp_min(1e-12)
