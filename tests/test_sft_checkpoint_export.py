...
class CheckpointExportTest(unittest.TestCase):
    def checkpoint(self, root, count=301):
        ...
        metadata = dict(schema='sensenova.u15.forge.sft.checkpoint.v3', trainer='torch_fsdp2',
                        samples_per_epoch=1000, max_samples=1000, consumed_samples=count,
                        optimizer_updates=10, last_update_samples=31,
                        checkpoint_target_samples=(count // 100) * 100, world_size=8, model_only=True,
    ...
    def test_cpu_roundtrip_and_latest_ignore_incomplete_staging(self):
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder); checkpoint, expected = self.checkpoint(root)
            ...
            for key in expected: torch.testing.assert_close(actual[key], expected[key])
    ...
    def test_failed_conversion_never_publishes_partial_target(self):
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder); checkpoint, _ = self.checkpoint(root)
