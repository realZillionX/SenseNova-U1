...
class ServingContractTest(unittest.TestCase):
    ...
    def test_client_separates_official_inference_from_rl_sampling(self) -> None:
        ...
        self.assertIn('--max_req_total_len "$MAX_SEQUENCE_LENGTH"', launcher)
        self.assertIn('FORGE_MAX_SEQUENCE_LENGTH="$MAX_SEQUENCE_LENGTH"', launcher)
        self.assertNotIn("MAX_REQ_TOTAL_LEN", launcher)
        self.assertIn('"max_sequence_length": 16384', client)
        self.assertIn('"max_images": 10', client)
        self.assertNotIn('"max_tokens"', client)
